...
$estResultado = showDataResultado($start_from, $record_per_page);
...
if ($leng != 0) {
    while ($i < $leng) {
        ...
        echo '<td id="fac_iia' . $newData[$i]['n_ins'] . '">' . $newData[$i]['ps'] . '</td>';
        echo '<td id="esc_iia' . $newData[$i]['n_ins'] . '">' . $newData[$i]['pca'] . '</td>';
        echo '<td id="car_iia' . $newData[$i]['n_ins'] . '">' . $newData[$i]['pcg'] . '</td>';
        echo '<td id="fac_iiia' . $newData[$i]['n_ins'] . '">' . $newData[$i]['gatb'] . '</td>';
        echo '<td id="esc_iiia' . $newData[$i]['n_ins'] . '">' . $newData[$i]['verbal'] . '</td>';
        echo '<td id="car_iiia' . $newData[$i]['n_ins'] . '">' . $newData[$i]['numer'] . '</td>';
        echo '<td id="car_iiia' . $newData[$i]['n_ins'] . '">' . $newData[$i]['indice'] . '</td>';
        echo '<td>
         <button type="button" title="Editar" id="edit_button' . $newData[$i]['n_ins'] . '" class="btn btn-warning btn-xs" onclick="edit_row(\'' . $newData[$i]['n_ins'] . '\');"><span class="glyphicon glyphicon-pencil"></span>    </button>
